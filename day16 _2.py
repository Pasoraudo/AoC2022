...
def cota_optimista(s):
    ...
    extra, i = 0, 0
    while i < len(not_used) and 0 < time and 0 < time_ele:
        yo = not_used[i]
        ...
        for ele_loc in valvulas[ele]:
            for yo_act in range(2):
                for ele_act in range(2):
                    ...
                    aux = (flow, yo_loc, ele_loc, time - 1 - yo_act, 
                    time_ele - 1 - ele_act, act_aux, not_used)
                    cota = cota_optimista(aux)
                    yield cota, aux
...
def solve(time):
    visited = {}
    not_used = []
    for k, v in valvulas.items():
        not_used.append((v, k))
        visited[k] = 0
    not_used = sorted(not_used, reverse=True)
    s = (0, 'AA', 'AA', time, time, visited, not_used)
    A = [(cota_optimista(s), s)]
    _heapify_max(A)
    mejor_flow = 1700 #1151  flow, yo, ele, my_time, ele_time, visitados
    while len(A) != 0:
        cota, s = heappop(A)
        if cota < mejor_flow:
            break
